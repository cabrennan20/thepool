...
const teamAbbreviationToESPN: Record<string, string> = {
  'ARI': 'ari',
  'ATL': 'atl',
  'BAL': 'bal',
  'BUF': 'buf',
...
const teamNameToESPN: Record<string, string> = {
 ...
 */
export function getTeamHelmetLogo(teamIdentifier: string, size: number = 500): string {
 ...
 */
export function isValidTeam(teamIdentifier: string): boolean {
 ...
 */
export function getAllTeams(size: number = 500): Array<{
  abbreviation: string;
  name: string;
  helmetsLogo: string;
}> {
