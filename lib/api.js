...
class ApiClient {
  ...
  async submitPicks(picks) {
    ...
    console.log('DEBUG - Submitting picks:', {
      pickCount: picks.length,
      picks: picks.slice(0, 3) // Log first 3 picks as sample
    });
  ...
  async getRecapData(week, season = new Date().getFullYear()) {
    try {
      ...
      // If it's a 403 error, recap is not available yet (picks still open)
      if (error.message.includes('Recap not available') || error.message.includes('picks are still open')) {
